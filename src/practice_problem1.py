...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        if len(self.contents+additional_contents)<=self.volume:
            ...
            number_of_new=self.volume-len(self.contents)
            for k in range(number_of_new):
                self.contents=self.contents+additional_contents[k]

            for k in range(number_of_new,len(additional_contents)):
                self.s=self.s+additional_contents[k]
        return self.s
    def double(self):
        """
        What comes in:
    ...
    def shrink(self, new_volume):
        ...
        if len(self.contents)>=new_volume:
            ...
            for k in range(new_volume):
                self.contents=self.contents+temp[k]
            for k in range(new_volume,len(temp)):
                self.s=self.s+temp[k]
